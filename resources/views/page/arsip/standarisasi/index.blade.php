...
@section('content')
    <x-arsip-tabs active="standarisasi" />


    <div class="d-flex justify-content-between align-items-center">
        <x-head-index :title="$title" />
        <x-btn-add :href="route('arsip.standarisasi.create')" title="Tambah Standarisasi" />
    </div>
                            ...
                            <td class="text-center">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{ route('arsip.standarisasi.edit', $item->id) }}"><i
                                                class="bx bx-edit-alt me-1"></i>
                                            Edit</a>

                                        <x-delete id="deleteStandarisasi{{ $item->id }}"
                                            action="{{ route('arsip.standarisasi.destroy', $item->id) }}"
                                            message="Yakin ingin menghapus standarisasi ini?" />
                                    </div>
                                </div>
                            </td>
                        </tr>
